...
class CleanerAgent(Agent):
    ...
    def getAction(self, state):
        ...
        for action in state.getLegalActions():
            for key in vector:
                if action == key and state.hasFood(pacman_pos[0] + vector[key][0], pacman_pos[1] + vector[key][1]):
                    return action
        return Directions.STOP
...
class FroggerAgent(Agent):
    ...
    def getAction(self, state):
        "The agent receives a GameState (defined in pacman.py)."
        "[Project 1] YOUR CODE HERE"
        #This pacman is a eater, he will search the nearest food.
        #This pacman is afraid of ghost, but he will calculate the road behind the ghost
        INTMAX = 100000
        vector = {Directions.EAST:(1, 0), Directions.WEST:(-1, 0), Directions.SOUTH:(0, -1), Directions.NORTH:(0, 1)}
        ...
        pacman_pos = state.getPacmanPosition()
        food = list(state.getFood())
        near_food_depth = 99999
        for x in range(len(food)):
            for y in range(len(food[x])):
                food_depth = abs(pacman_pos[0] - x) + abs(pacman_pos[1] - y)
                if food[x][y] and near_food_depth > food_depth:
                    near_food_depth = food_depth
                    food_pos = (x, y)
        bfs_map = [[0 for y in range(len(grid[x]))] for x in range(len(grid))]
        ...
        for ghost_pos in state.getGhostPositions():
            ...
            grid[ghost_pos[0]][ghost_pos[1]] = False
            bfs_map[ghost_pos[0]][ghost_pos[1]] = 0
            for key in vector:
                x = ghost_pos[0] + vector[key][0]
                y = ghost_pos[1] + vector[key][1]
